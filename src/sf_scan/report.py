...
def _build_ontology_index(
    ...
    for finding, path in mapped:
        ...
        prd_entry = index.setdefault(
            ...
            {
                "title": prd.title,
                "path": _link_to(out_dir, prd.path),
                "features": {},
        ...
        if feature is None:
            ...
            feature_entry = prd_entry["features"].setdefault(
                ...
                {
                    "title": feature.title,
                    "path": _link_to(out_dir, feature.path),
                    "blueprints": {},
                },
            )
...
def _node_summary(node: KGNode | None, out_dir: Path) -> dict[str, Any] | None:
    ...
    return {
        "id": node.id,
        "title": node.title,
        "path": _link_to(out_dir, node.path),
    }
...
def _render_mapped_findings(
    ...
    for prd_id in sorted(grouped.keys()):
        ...
        out.append(f"### [{prd_node.id}: {prd_node.title}]({_link_to(out_dir, prd_node.path)})")
        out.append("")
        ...
        for bp_id in sorted(prd_data["foundations"]):
            ...
            bp_node = f_data["node"]
            out.append(
                f"#### Foundation: [{bp_node.id}: {bp_node.title}]"
                f"({_link_to(out_dir, bp_node.path)})"
            )
            out.append("")
            for wo_id in sorted(f_data["work_orders"]):
                ...
                out.append(
                    f"##### [{wo_node.id}: {wo_node.title}]"
                    f"({_link_to(out_dir, wo_node.path)})"
                )
                out.append("")
                for f in sorted(
        ...
        for feature_id in sorted(prd_data["features"]):
            ...
            out.append(
                f"#### Feature: [{feat_node.id}: {feat_node.title}]"
                f"({_link_to(out_dir, feat_node.path)})"
            )
            out.append("")
            for bp_id in sorted(feat_data["blueprints"]):
                ...
                out.append(
                    f"##### Blueprint: [{bp_node.id}: {bp_node.title}]"
                    f"({_link_to(out_dir, bp_node.path)})"
                )
                out.append("")
                for wo_id in sorted(bp_data["work_orders"]):
                    ...
                    wo_node = wo_data["node"]
                    out.append(
                        f"###### Work Order: [{wo_node.id}: {wo_node.title}]"
                        f"({_link_to(out_dir, wo_node.path)})"
                    )
                    out.append("")
                    for f in sorted(
